sigex.resid <- function(psi,mdl,data)
{
	...
	#		by Tucker McElroy
	#
	#   Computes residuals of a latent multivariate component process
	#		psi = [xi,zeta,beta]
	...
	#	returns Nx(T-d) matrix of residuals
	#		
	##############################
	...
	if(!inherits(attempt, "try-error")) {
		lik.output <- attempt[[2]] } else lik.output <- Inf
