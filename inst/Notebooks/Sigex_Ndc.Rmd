...
Manufacturing, Shipments, and Inventories survey.
(This is seasonally adjusted monthly data covering 
 the sample period January 1992 to April 2020,  downloaded July 21, 2020 (4:45 PM),
  U.S.  Census Bureau, obtained from
 https://www.census.gov/mtis/index.html by selecting 
...
- To do this, we are careful to omit the first observation (due to the missing value), and difference the time series to remove trend non-stationarity.
...
- But we must first arrange the VAR parameter estimates in the right order, which requires some permuting.
- Also the innovation covariance matrix is broken into its Cholesky decomposition using **getGCD**.  This function outputs a list object, including the lower triangular entry of $L$ and the log of the diagonal entries of $D$.
...
- Next, the array of VAR parameters are mapped via **var.par2pre** to a pre-parameter vector  *var.out* of length 28.
- Then *psi.init* is assembled from the Yule-Walker fit, including the sample means, which are incorporated as the last two pre-parameters.
...
- The third argument  gives the model type as  *varma*;  
  the order $p=7$, $q=0$ of the VARMA is given in the fourth argument.  
...
- The output indicates there is little  residual autocorrelation,  which is confirmed by inspection of the sample    autocorrelation.
...
- There is some non-normality indicated.  However, we are not principally interested in the marginal distributions of our time series model.
...
- Now the modeling and fitting is completed, we can do forecasting and midcasting.
...
- Midcast for the missing value at the first time point is automatically generated.
...
- The output of **sigex.midcast** is a list with two items:  the matrix of casts, and a 4-array of casting error  covariances.   
- Note that these casts are for the series with all fixed effects removed. 
- This output can be formatted for use via  **sigex.castextract**,  which inserts the casts in the correct      places in the time series and adds an extended trend regression effect (it is extended forward and backward      by the *castspan* value).  
...
- For each component time series, the raw data is plotted  after first padding with NA
  before and after.    
